...
const DiceRoll = ({ board, player }) => {
    ...
    function renderDieFace(value) {
        return (
            <div className={`die ${animating ? 'rolling' : ''}`}>
                {[...Array(value)].map((_, i) => (
                    <div key={i} className={`dot dot-${i + 1} ${getDotPositionClass(value, i + 1)}`}></div>
                ))}
            </div>
        );
    };
    ...
    function getDotPositionClass(value, dotNum) {
        switch (value) {
            ...
            case 3:
                return dotNum === 1 ? 'top-left' : dotNum === 2 ? 'center' : 'bottom-right';
            case 4:
                return dotNum === 1 ? 'top-left' : dotNum === 2 ? 'top-right' :
                    dotNum === 3 ? 'bottom-left' : 'bottom-right';
            case 5:
                return dotNum === 1 ? 'top-left' : dotNum === 2 ? 'top-right' :
                    dotNum === 3 ? 'center' : dotNum === 4 ? 'bottom-left' : 'bottom-right';
            case 6:
                return dotNum === 1 ? 'top-left' : dotNum === 2 ? 'top-right' :
                    dotNum === 3 ? 'middle-left' : dotNum === 4 ? 'middle-right' :
                        dotNum === 5 ? 'bottom-left' : 'bottom-right';
            default:
                return '';
        }
    };
